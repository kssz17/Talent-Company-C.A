  ...
  <div style="min-height:100vh;background:var(--bg);">
    ...
    >
        ...
        <div class="flex items-center h-14 gap-6">
          ...
          >
            ...
            <div class="hidden sm:block text-left">
              <p class="text-sm font-bold leading-none" style="color:var(--text-1);">Talent Company</p>
              <p class="text-[10px] mt-0.5 leading-none" style="color:var(--text-3);">Portal de Empleo</p>
            </div>
          </button>

          <div class="hidden md:block h-5 w-px" style="background:var(--border);" />
          ...
          <nav class="hidden md:flex items-center gap-1 flex-1">
            <button
              ...
              :class="{ 'portal-nav-item--active': isActive(item.name) }"
          ...
          <div class="flex items-center gap-3 ml-auto">
            <div class="hidden sm:block text-right">
              <p class="text-xs font-semibold leading-none" style="color:var(--text-1);">{{ auth.displayName }}</p>
              <p class="text-[10px] mt-0.5" style="color:var(--text-3);">Candidato</p>
            </div>
            <!-- Avatar -->
            <div
            ...
            <button class="logout-btn hidden sm:flex items-center gap-1" @click="handleLogout">
            ...
            <!-- Mobile hamburger -->
            <button class="mobile-btn md:hidden p-1.5 rounded-md" @click="mobileOpen = !mobileOpen">
      ...
      <Transition name="fade-down">
        <div
          v-if="mobileOpen"
          class="md:hidden px-4 py-2"
          style="border-top:1px solid var(--border);background:var(--surface);"
        >
          <button
            v-for="item in navItems"
            :key="item.name"
            class="portal-nav-item w-full mb-0.5"
            :class="{ 'portal-nav-item--active': isActive(item.name) }"
          ...
          <button class="logout-btn w-full mt-1 flex items-center gap-2" @click="handleLogout">
...
.portal-nav-item:hover { background: rgba(255,255,255,0.05); color: var(--text-1); }
.portal-nav-item--active { background: var(--accent-m); color: var(--accent); font-weight: 600; }
...
.mobile-btn:hover { background: rgba(255,255,255,0.05); color: var(--text-1); }
