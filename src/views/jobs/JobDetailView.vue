...
<template>
  <div v-if="!job" class="text-center py-20">
    <p style="color:var(--text-2);">Oferta no encontrada.</p>
    ...
    <div class="grid lg:grid-cols-3 gap-5">
        ...
        <div class="card p-6">
          <div class="flex items-start justify-between gap-3 mb-4">
            <div>
              ...
              <h2 class="text-xl font-semibold mt-2" style="color:var(--text-1);">{{ job.title }}</h2>
              <p class="text-sm mt-1" style="color:var(--text-2);">{{ job.department?.name }}</p>
            </div>
          </div>
          ...
          <div>
            <h4 class="text-sm font-semibold mb-2" style="color:var(--text-1);">Descripción</h4>
            <p class="whitespace-pre-line text-sm leading-relaxed" style="color:var(--text-2);">{{ job.description }}</p>
          </div>

          <div v-if="job.requirements" class="mt-5">
            <h4 class="text-sm font-semibold mb-2" style="color:var(--text-1);">Requisitos</h4>
            <p class="whitespace-pre-line text-sm leading-relaxed" style="color:var(--text-2);">{{ job.requirements }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="space-y-4">
        ...
        <div v-if="auth.isRecruiter" class="card p-4 space-y-2">
          <p class="text-xs font-medium uppercase tracking-wide" style="color:var(--text-3);">Cambiar estado</p>
        ...
        <div class="card p-4 space-y-3">
          <p class="text-xs font-medium uppercase tracking-wide" style="color:var(--text-3);">Estadísticas</p>
          <div class="flex justify-between text-sm">
            <span style="color:var(--text-2);">Candidaturas</span>
            <span class="font-semibold" style="color:var(--text-1);">{{ applications.length }}</span>
          </div>
          <div class="flex justify-between text-sm">
            <span style="color:var(--text-2);">Publicada</span>
            <span class="font-medium" style="color:var(--text-1);">{{ job.published_at?.slice(0,10) ?? '—' }}</span>
          </div>
          <div class="flex justify-between text-sm">
            <span style="color:var(--text-2);">Creada por</span>
            <span class="font-medium" style="color:var(--text-1);">{{ job.creator?.full_name }}</span>
          </div>
        </div>

        <!-- Recent candidates -->
        <div class="card p-4">
          <p class="text-xs font-medium uppercase tracking-wide mb-3" style="color:var(--text-3);">Candidatos recientes</p>
          <div class="space-y-2">
            <div
              v-for="app in applications.slice(0, 4)"
              :key="app.id"
              class="flex items-center gap-2 cursor-pointer rounded-lg p-1.5 -mx-1.5 transition-colors"
              style="transition:background .1s;"
              @mouseenter="($el as HTMLElement).style.background = 'rgba(255,255,255,0.04)'"
              @mouseleave="($el as HTMLElement).style.background = ''"
